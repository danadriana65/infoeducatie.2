...
class LoginScreen(Screen):
    def _init_(self, **kwargs):
        ...
        self.add_widget(self.layout)
        print(os.path.exists("WhatsApp Image 2025-07-02 at 21.05.20_a0c9f2ee.png"))
        # 🎬 Video pe fundal
    ...
    def login(self, instance):
        ...
        if verify_user(entered_username, entered_password):
            ...
            # 🛰 Tranziție către ecranul principal
...
class UserDataManager:
    def _init_(self, file_path="user_data.json"):
...
class NavigateScreen(Screen):
    def _init_(self, option, user_data, user_input, user_mail, **kwargs):
        super().__init__(**kwargs)
        self.option = option
...
class OptionsScreen(Screen):
    def _init_(self, planet_name, selected_option, **kwargs):
        self.planet_name = kwargs.pop("planet_name")
        self.selected_option = kwargs.pop("selected_option")
        super().__init__(**kwargs)
...
answers_python_b = [
    ...
    ["[x*2 for x in range(10)]", "for x in range(10):\n    x2", "[for x in range(10): x2]", "map(lambda x: x*2, range(10))"],
...
answers_python = [
    ["O metodă care creează o instanță înainte de _init_()", "O metodă folosită doar în moștenirea multiplă", "O metodă specială care returnează self", "O metodă de conversie între tipuri de date"],
    ...
    ["Creează o variabilă globală numită _main_", "Permite executarea în paralel a mai multor funcții", "Determină dacă scriptul este executat direct", "Verifică dacă scriptul este importat din alt modul"],
    ["threading.Event()", "threading.pause()", "threading.Lock()", "threading.Semaphore()"],
    ["True", "False", "Va genera o eroare", "None"]
]

correct_answers_python = ["O metodă care creează o instanță înainte de _init_()", "Definește o metodă statică care nu are acces la self", "copy() creează o copie superficială, iar deepcopy() copiază recursiv obiectele", "[0, 1, 4]", "Determină dacă scriptul este executat direct", "threading.pause()", "True"]
questions_cpp = [
...
class Image2Screen(Screen):
    def _init_(self, image_path, next_screen, question_data, **kwargs):
        super().__init__(**kwargs)
...
class QuestionScreen(Screen):
    def _init_(self, question_data, **kwargs):
        super().__init__(**kwargs)
        self.question_data = question_data
        self.buttons = []
...
def open_profile_popup():
    ...
    if image_path and os.path.exists(image_path):
        try:
            ...
            layout.add_widget(Label(text=f"⚠ Eroare la poză: {e}"))
    else:
