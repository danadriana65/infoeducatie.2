...
def show_grid():
    ...
    background_video_path =get_asset_path(script_dir / "Data/Images/Background.mp4")  # Default video file for background
...
def store_user_info(username, password, email):
    user_data = load_user_credentials()  # Încărcăm datele existente
...
def login():
    ...
    user_data = load_user_credentials()  # ✅ Încărcăm datele salvate
...
def navigate_to_page(option):
    ...
    planet_images = {
        "Uranus": PhotoImage(file=script_dir / "Data/Images/Uranus.png"),  # Replace paths
        "Venus": PhotoImage(file=script_dir / "Data/Images/Venus.png"),
        "Saturn": PhotoImage(file=script_dir / "Data/Images/Saturn.png"),}
...
questions_cpp1 = df[["questions_cpp1"]].values.tolist()

# Vectorul de cod Python asociat fiecărei întrebări
code_cpp1 = df[["code_cpp1"]].values.tolist()
answers_cpp1 = [
...
def handle_answer(selected_answer, button, planet_name, selected_option, correct_answer):
    ...
    if selected_answer == correct_answer:
        ...
        play_question_video(script_dir / "Data/Images/Correct.mp4", show_question_page, planet_name, selected_option)
    else:
        button.config(bg="red")
        play_question_video(script_dir / "Data/Images/Wrong.mp4", show_question_page, planet_name, selected_option)
...
def save_credentials(username, password, email):
    file_path = script_dir / "Data/Assets/credentials.txt"
    try:
